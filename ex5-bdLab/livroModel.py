...
class LivroModel:
    ...
    def update_livro(self, _id:int, titulo: str, autor: str, ano: int, preco: float):
        try:
            res = self.db.collection.update_one({"_id": _id}, {"$set": {"titulo": titulo, "autor": autor, "ano": ano, "preco": preco}})
    ...
    def delete_livro(self, _id: int):
        try:
            res = self.db.collection.delete_one({"_id": _id})
